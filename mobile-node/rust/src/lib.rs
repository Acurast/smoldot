...
use core::{num::NonZero, pin::Pin, str, sync::atomic::Ordering, task};
use std::ptr::null;
...
type Client = init::Client<Arc<smoldot_light::platform::DefaultPlatform>, ()>;

fn client() -> &'static Mutex<Option<Client>> {
    static CLIENT: async_lock::OnceCell<Mutex<Option<Client>>> = async_lock::OnceCell::new();

    CLIENT.get_or_init_blocking(|| Mutex::new(None))
}

/// Runs `f` with exclusive access to the client, creating the client first if it doesn't exist.
fn with_client<R>(f: impl FnOnce(&mut Client) -> R) -> R {
    let mut client_lock = futures_lite::future::block_on(client().lock());
    let client = client_lock.get_or_insert_with(|| init::Client {
...
/// Drops the client together with all its chains and services, and clears the panicked state.
/// The next [`with_client`] call re-creates the client from scratch.
fn reset_client() {
    let mut client_lock = futures_lite::future::block_on(client().lock());
    *client_lock = None;

    platform::POISONED.store(false, Ordering::SeqCst);
}
...
fn add_chain(
    ...
    let (outer_chain_id, potential_relay_chains) = with_client(|client| {
            ...
            potential_relay_chains
                ...
                .filter_map(|c| {
                    if let Some(init::Chain::Created {
                        smoldot_chain_id, ..
                    }) = client.chains.get(usize::try_from(c).ok()?)
                    {
        ...
        let outer_chain_id = client.chains.insert(init::Chain::Initializing);

        (outer_chain_id, potential_relay_chains)
    });
        ...
        async move {
            with_client(|client| {
                ...
                let smoldot_light::AddChainSuccess {
                    ...
                    Ok(c) => c,
                    Err(error) => {
                        client.chains.remove(outer_chain_id);
                ...
                client.chains[outer_chain_id] = init::Chain::Created {
                    smoldot_chain_id,
                ...
                if let init::Chain::Created {
                    ...
                    ..
                } = client.chains.get_mut(outer_chain_id).unwrap()
                {
    ...
    with_client(|client| {
            ...
            .try_remove(usize::try_from(chain_id).unwrap())
        {
            Some(init::Chain::Created {
                ...
                let () = client.smoldot.remove_chain(smoldot_chain_id);
            }
...
fn json_rpc_send(json_rpc_request: Vec<u8>, chain_id: u32) -> u32 {
    ...
    with_client(|client| {
        let client_chain_id = match client
            .chains
